...
function networkUp () {
  ...
  if [ "${IF_COUCHDB}" == "couchdb" ]; then
      IMAGE_TAG=${IMAGETAG} docker-compose -f $COMPOSE_FILE_ORG -f $COMPOSE_FILE_ORG_COUCH up -d 2>&1
  else
...
function generateChannelArtifacts() {
  ...
  (mkdir -p ${ORG_NAME}-artifacts
   ...
   sed -e "s/\${ORG_NAME}/${ORG_NAME}/g" \
     ...
     -e "s/\${ORG_DOMAIN}/${ORG_DOMAIN}/g" \
     -e "s/\${PORT}/${PORT}/g" \
     org-artifacts-template/configtx.yaml > ${ORG_NAME}-artifacts/configtx.yaml
   cd ${ORG_NAME}-artifacts
   export FABRIC_CFG_PATH=$PWD
   set -x
   configtxgen -printOrg ${ORG_MSPID} > ../channel-artifacts/${ORG_NAME}.json
   res=$?
   set +x
   if [ $res -ne 0 ]; then
